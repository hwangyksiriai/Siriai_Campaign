...
// 상태값 정규화: 한글 '모집예정'/'예정' → 'upcoming'
function normStatus(s) {
  const v = String(s || "open").trim().toLowerCase();
  return v === "모집예정" || v === "예정" ? "upcoming" : v;
}
  ...
  const toLines = (v) =>
    Array.isArray(v)
      ...
      : typeof v === "string"
        ...
        : [];
  const slug = String(c.slug || "").trim() || slugify(c.title);
  return {
    ...c,
    slug,
    // 이미지를 따로 지정 안 했으면 /campaigns/<slug>.png 를 자동으로 사용
    // (파일이 없으면 CoverImage가 브랜드 이니셜로 대체)
    cover_image_url:
      String(c.cover_image_url || "").trim() || `/campaigns/${slug}.png`,
